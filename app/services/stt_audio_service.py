...
def _writable_cookie_file() -> str | None:
    ...
    if not _COOKIE_PATH.exists():
        return None
    # Always copy to a throwaway temp path and point yt-dlp there. The mounted
    # cookies file must stay read-only and pristine: yt-dlp rewrites the cookie
    # jar on close (YouTube rotates session cookies), and letting it write back
    # to the source corrupts the session across the pipeline's repeated calls.
    # Writing to /tmp also avoids OSError(Errno 30) on the read-only mount.
